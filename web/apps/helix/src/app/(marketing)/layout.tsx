...
const MarketingLayout = async ({ children }: Readonly<{ children: React.ReactNode }>) => {
  ...
  return (
    // `clip`, never `hidden`: `overflow-x: hidden` forces `overflow-y: auto`, making this div the
    // scrollport for every `sticky` descendant — so the header and post TOC would never stick.
