...
int main(int argc, char **argv)
{
    ...
    // On-demand overlaid clip recorder for stream REC_CAM: x264(sw) -> in-memory GOP ring.
    if (system("mkdir -p /home/radxa/clips") != 0)
    {
    }
    {
        GError *re = NULL;
    ...
    while (running)
    {
        ...
        if (t - tlast >= 2000)
        {
            ...
            {
                tot += g_inf_m[m];
                fprintf(stderr, " %s=%.1f inf/s(%ld det)", MODELS[m].name, g_inf_m[m] / el, g_det_m[m]);
            }
            fprintf(stderr, " | aggregate=%.1f inf/s\n", tot / el);
            tlast = t;
        }
    }
