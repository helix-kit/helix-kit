...
void awnn_run_hw(Awnn_Context_t *info) {
    ...
    if (vip_run_network(info->network) != VIP_SUCCESS)
        ALOGE("awnn_run_hw: vip_run_network failed\n");
    pthread_mutex_unlock(&info->mutex);
}
void awnn_finish(Awnn_Context_t *info) {
    int i, j;
    pthread_mutex_lock(&info->mutex);
    for (i = 0; i < info->output_count; i++)
        vip_flush_buffer(info->output_buffers[i], VIP_BUFFER_OPER_TYPE_INVALIDATE);
    for (i = 0; i < info->output_count; i++) {
        int buff_size = vip_get_buffer_size(info->output_buffers[i]);
        void *out_data = vip_map_buffer(info->output_buffers[i]);
        vip_buffer_create_params_t *param = &info->output_params[i].vip_param;
        vip_uint32_t elements = info->output_params[i].elements;
        float *fp_data = info->user_output_buffers[i];
        if (param->data_format == VIP_BUFFER_FORMAT_FP32) {
            memcpy(fp_data, out_data, buff_size);
        } else if (param->data_format == VIP_BUFFER_FORMAT_FP16) {
            short *data = (short*)malloc(buff_size); memcpy(data, out_data, buff_size);
            for (j = 0; j < (int)elements; j++) fp_data[j] = fp16_to_fp32(*(data + j));
            free(data);
        } else if (param->data_format == VIP_BUFFER_FORMAT_UINT8
                || param->data_format == VIP_BUFFER_FORMAT_INT8) {
            unsigned char *data = (unsigned char*)malloc(buff_size); memcpy(data, out_data, buff_size);
            for (j = 0; j < (int)elements; j++) fp_data[j] = info->quantize_maps[i][*(data + j)];
            free(data);
        } else if (param->data_format == VIP_BUFFER_FORMAT_INT16) {
            short *data = (short*)malloc(buff_size); memcpy(data, out_data, buff_size);
