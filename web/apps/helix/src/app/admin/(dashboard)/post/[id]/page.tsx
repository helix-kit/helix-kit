...
const Editor = dynamic(() => import('@/components/editor/mdx-editor'), { ssr: false });
...
const EditPostPage = () => {
  ...
  const query = useTRPCQuery((api) =>
    api.blog.getById.queryOptions({ id }, { refetchOnMount: false }),
  );
  const update = useTRPCMutation((api) => api.blog.update.mutationOptions());
