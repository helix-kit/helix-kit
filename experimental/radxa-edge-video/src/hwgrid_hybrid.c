...
int main(int argc, char** argv) {
    ...
    snprintf(desc, sizeof(desc),
        ...
        " rtspsrc location=rtsp://%s:8554/stream1 protocols=tcp latency=200 ! rtph264depay ! h264parse ! omxh264dec ! videoconvert ! video/x-raw,format=NV12 ! queue ! mix.sink_0 "
        " rtspsrc location=rtsp://%s:8554/stream2 protocols=tcp latency=200 ! rtph264depay ! h264parse ! omxh264dec ! videoconvert ! video/x-raw,format=NV12 ! queue ! mix.sink_1 "
        " rtspsrc location=rtsp://%s:8554/stream3 protocols=tcp latency=200 ! rtph264depay ! h264parse ! omxh264dec ! videoconvert ! video/x-raw,format=NV12 ! queue ! mix.sink_2 "
        " rtspsrc location=rtsp://%s:8554/stream4 protocols=tcp latency=200 ! rtph264depay ! h264parse ! omxh264dec ! videoconvert ! video/x-raw,format=NV12 ! queue ! mix.sink_3 ",
        host, host, host, host);
