...
| `@helix-hq/pdf-report/editor` | `ReportTemplateProvider`, `useReportTemplate`, the five field components, `ReportPreview` / `useReportPreview`, and `ReportTemplateEditor` — one ready-made composition of them |
...
The editor is not one component. `ReportTemplateProvider` owns the drafts and
renders nothing; each field reads it through context; `ReportPreview` renders
whatever the drafts currently parse to. A host assembles the ones it wants:

```tsx
<ReportTemplateProvider value={template} onChange={setDraft} onError={setError}>
  <ReportCodeField />
  <ReportOutputSchemaField />
  <ReportLayoutField />
  <ReportPreview input={realData} parseError={error} renderMode="server" />
</ReportTemplateProvider>
```

That example is a real case: an application whose input shape is fixed leaves
`ReportInputSchemaField` out — the schema still types `input` in the code field,
it simply stops being editable — and passes real values to `ReportPreview`
instead of shipping a stored sample, which makes `ReportDemoInputField`
unnecessary too. A preview is only worth trusting when it runs on the values a
recipient will actually see.

`useReportPreview` is the same rendering without the iframe, for hosts that want
to present the result differently. `ReportTemplateEditor` remains as the
all-five-fields-plus-preview composition, and is written in terms of the pieces
